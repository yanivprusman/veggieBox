...
import com.automatelinux.veggieBox.util.AppStrings
...
private val tabs = listOf(
    Tab("route", AppStrings.tabRoute, Icons.AutoMirrored.Filled.List),
    Tab("map", AppStrings.tabMap, Icons.Filled.Map),
    Tab("greeting", AppStrings.tabGreeting, Icons.Filled.MarkChatUnread),
)
...
fun AppNavGraph() {
    ...
    // Record the current screen so a feedback report knows where the user was.
    LaunchedEffect(route) {
        ScreenTracker.currentScreen = when {
            route == "route" -> AppStrings.tabRoute
            route == "map" -> AppStrings.tabMap
            route == "greeting" -> AppStrings.tabGreeting
            route?.startsWith("stop") == true -> "פרטי משלוח"
            else -> route
        }
    }

    // PT-style floating feedback widget (dev flavor only) + new-version snackbar.
    FeedbackOverlay(
